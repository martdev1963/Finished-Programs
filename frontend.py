...
database=Database("books.db")  # database object is sent to self parameter and "books.db" is sent to db parameter...
...
def view_command():
    ...
    for row in database.view(): # view() method returns a list of tuples...
        list1.insert(END,row) # insert list of tuples into listbox object...which will then be viewable to the user...

# b2=Button(window,text="Search Entry"
def search_command():
    list1.delete(0,END)
    for row in database.search(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()): # stringVar objects getting strings from user entry...
        list1.insert(END,row) # insert the  found strings into the listbox object...

# b3=Button(window,text="Add Entry"
def add_command():
    database.insert(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()) # insert new entry into database...
...
def delete_command(): # passing global variable selected_tuple[0]...
    database.delete(selected_tuple[0]) # will delete the selected_tuple at index[0] for that single tuple returned...
...
def update_command(): # getting the updated data from the entry fields and updating the database...notice the get() methods...
    database.update(selected_tuple[0],title_text.get(),author_text.get(),year_text.get(),isbn_text.get()) # will delete the selected_tuple at index[0] for that single tuple returned...
    print(selected_tuple[0],selected_tuple[1],selected_tuple[2],selected_tuple[3],selected_tuple[4]) # just prints to the console for debugging purposes...
...
l0=Label(window,text="Martin Batista Python Prgrmr - Guavadream Media Software 2018 ")
...
# scrollbar object and its x y coordinates...
